...
│  UNSW-NB15 Dataset (257,673건 네트워크 패킷)              │
│  42 Features: dur, proto, service, state, rate, load...  │
└──────────────────┬──────────────────────────────────────┘
                   │
┌──────────────────▼──────────────────────────────────────┐
│           AI/ML 학습 계층 (Training Layer)                │
│  XGBoost Classifier                                      │
│  점진적 학습: Day1~Day10 모델 순차 학습                   │
│  자동 Best 모델 선택 (FPR+FNR 최소화)                     │
└──────────────────┬──────────────────────────────────────┘
                   │
┌──────────────────▼──────────────────────────────────────┐
│          실시간 탐지 계층 (Detection Layer)                │
│  PacketProvider: 실제 데이터셋에서 패킷 샘플링            │
│  Feature Extraction: 42개 특성 추출                       │
│  Attack Classification: 9가지 공격 유형 분류              │
└──────────────────┬──────────────────────────────────────┘
                   │
┌──────────────────▼──────────────────────────────────────┐
│         지능형 분석 계층 (Analysis Layer)                  │
│  AttackAnalyzer: Feature 기반 판단 근거 생성              │
│  심각도 자동 평가 (0-10 스케일)                           │
│  AI 기반 대응 방안 자동 생성                              │
│  [선택] OpenAI API 연동 고급 분석                         │
└──────────────────┬──────────────────────────────────────┘
                   │
┌──────────────────▼──────────────────────────────────────┐
│      시각화 및 모니터링 계층 (Monitoring Layer)            │
│  Frontend Dashboard: 실시간 공격 모니터링                 │
│  Admin Panel: 공격 시뮬레이션 및 성능 추적                │
│  Performance Metrics: FPR/FNR/Accuracy 추적              │
└─────────────────────────────────────────────────────────┘
```
